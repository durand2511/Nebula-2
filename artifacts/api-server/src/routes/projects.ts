...
import { checkWritePlanViolation, isNewPageOnImportedSite, importedSiteHasEdits } from "../lib/write-plan.js";
...
(function(){
  ...
  if (page !== "booking-app.html") {
    ...
    // Ticker/marquee bars scroll via a small self-contained script (their own JS doesn't load here).
    html = /<\/body>/i.test(html) ? html.replace(/<\/body>/i, TICKER_SCRIPT + "</body>") : html + TICKER_SCRIPT;
...
CRITICAL RESTYLE QUALITY RULES (these break sites when ignored):
- NAV/HEADER must stay COMPACT: modest vertical padding (~10–16px), normal height. Do NOT turn it into a big floating pill/box with huge padding or margins that push content down or overlap it.
- DROPDOWN / SUB-MENU readability: if you give a submenu (.sub-menu / .elementor-sub-item / .dropdown-menu) a background, set an explicit CONTRASTING text colour on its links so items are clearly readable (never dark text on a dark box). Give it enough padding and a solid (not transparent) background.
- Never make hidden helper elements visible: do NOT restyle .screen-reader-text, .skip-link, skip-to-content links, or [hidden] — leave them hidden.
- Keep it RESPONSIVE and don't cause overlap: use relative units and don't set fixed heights that clip text.`
...
// The real-AI edit path. Runs the Claude Agent SDK against a temp checkout of the
// project (see lib/agent-editor.ts) and streams its actions to the build session.
async function runBuildPipeline(
  ...
  try {
    ...
    // Snapshot the pre-edit state so the user can "draai dit terug" afterwards.
    const existing = await db
      .select({ id: projectFiles.id })
      .from(projectFiles)
      .where(eq(projectFiles.projectId, projectId))
      .limit(1);
    if (existing.length > 0) await snapshotProject(projectId, content.slice(0, 120));

    const result = await runAgentEdit({ projectId, prompt: content, images, emit: send, abortController });
    ...
    const touched = result.changed.length + result.created.length + result.deleted.length;
    const reply = result.finalText.trim()
      || (touched > 0 ? "Klaar — de wijzigingen zijn doorgevoerd." : "Ik heb geen wijzigingen aangebracht.");
    await db.insert(projectMessages).values({ projectId, role: "assistant", content: reply });

    if (!result.ok && touched === 0) {
      send({ type: "error", message: "Het is niet gelukt om de wijziging door te voeren. Probeer het opnieuw of formuleer het anders." });
      finishBuildSession(session, "error");
      return;
    }

    send({ type: "done", files: [...result.changed, ...result.created] });
    finishBuildSession(session, "done");
  } catch (err) {
    if (abortController.signal.aborted) {
      send({ type: "done", files: [], cancelled: true });
    ...
    logger.error({ err, projectId }, "runBuildPipeline (agent) failed");
    send({ type: "error", message: "Er is iets misgegaan tijdens het bewerken." });
    finishBuildSession(session, "error");
  }
}
...
User request: "${text.slice(0, 400)}"`,
    ...
    // The Agent SDK is now the editor for all content changes (text, colours, fonts, nav,
    // pages, images, sections). Only the actions that MUST stay deterministic — the hard-coded
    // booking subsystem, login seeding, external-booking removal and undo — are handled here as
    // instant shortcuts; everything else falls through to the AI so a normal edit uses the agent.
    const KEEP_DETERMINISTIC = new Set(["add_booking_app", "set_booking_logins", "remove_external_bookings", "undo"]);
    return KEEP_DETERMINISTIC.has(action.action) ? action : { action: "none", reason: "prefer AI editor" };
  } catch (err) {
