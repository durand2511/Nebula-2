...
const MOBILE_MENU_SCRIPT = `<script>(function(){document.addEventListener("click",function(e){var el=e.target;var t=el&&el.closest?el.closest('.elementor-menu-toggle,[class*="menu-toggle"]'):null;if(!t)return;e.preventDefault();var open=!t.classList.contains("elementor-active");t.classList.toggle("elementor-active",open);t.setAttribute("aria-expanded",open?"true":"false");var scope=t.closest("nav,.elementor-widget-nav-menu,.elementor-nav-menu--main,header")||document;var dd=scope.querySelector(".elementor-nav-menu--dropdown")||scope.querySelector(".elementor-nav-menu__container")||scope.querySelector("ul.elementor-nav-menu")||scope.querySelector(".sub-menu");if(!dd)return;if(open){var r=t.getBoundingClientRect();var topY=Math.max(0,Math.round(r.bottom));dd.style.display="block";dd.style.position="fixed";dd.style.top=topY+"px";dd.style.left="0";dd.style.right="0";dd.style.width="100%";dd.style.maxHeight=(window.innerHeight-topY-8)+"px";dd.style.overflowY="auto";dd.style.zIndex="99999";var bg="";try{bg=getComputedStyle(dd).backgroundColor;}catch(x){}if(!bg||bg==="rgba(0, 0, 0, 0)"||bg==="transparent"){dd.style.background="#fff";}}else{["display","position","top","left","right","width","maxHeight","overflowY","zIndex","background"].forEach(function(p){dd.style[p]="";});}},true);})();</script>`;
