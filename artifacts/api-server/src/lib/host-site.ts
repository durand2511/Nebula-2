...
document.addEventListener("click",function(e){var t=e.target&&e.target.closest?e.target.closest('.elementor-menu-toggle,[class*="menu-toggle"]'):null;if(!t)return;e.preventDefault();e.stopPropagation();var dd=findDd(t);if(!dd)return;var open=dd.getAttribute("data-nebmenu")!=="1";dd.setAttribute("data-nebmenu",open?"1":"0");t.classList.toggle("elementor-active",open);t.setAttribute("aria-expanded",open?"true":"false");dd.setAttribute("aria-hidden",open?"false":"true");if(open)show(t,dd);else hide(dd);},true);
})();</script>`;
