...
## Escaped `<noscript>` lazy-load fallbacks print as visible text
WordPress lazy-load also emits `<noscript><img src="real" ...></noscript>` for no-JS users. The importer parsed these with scripting-enabled semantics, so the `<noscript>` body became a TEXT node and got stored HTML-escaped (`&lt;img ...&gt;`). Scripts run in our preview, so the browser prints those raw tags as visible text UNDER each image. `buildPreviewHtml` strips escaped `&lt;img ...&gt;` fallbacks, scoped to ones containing `lazyload` so legit escaped code shown by a generated app is left alone. (Symptom the user reports: "I see `<img .../>` text under the images.")
